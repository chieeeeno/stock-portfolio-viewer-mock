...
2. **Given** 銘柄一覧が表示されている、**When** ユーザーが各銘柄を見る、**Then** 各銘柄のロゴ画像、銘柄名、ティッカーシンボル、評価損益額（円）、評価損益率（%）が表示される
...
- **FR-006**: パイチャートの下に保有銘柄の一覧を表示し、各銘柄には銘柄ロゴ画像、銘柄名、ティッカーシンボル、評価損益額（円）、評価損益率（%）を含めなければならない
