...
## フェーズ9.6: ユーザーストーリー6A - ユーザーメニュー (優先度: P3)

**ゴール**: 顧客がヘッダーのユーザーアイコンをクリックすることで、ユーザー関連の操作メニュー（ドロップダウン）を表示できる

**独立テスト**: ユーザーアイコンをクリックするとドロップダウンメニューが表示され、メニュー外をクリックすると閉じる

**使用ライブラリ**: shadcn/ui Dropdown Menu（既存のshadcn/ui Tooltipとの整合性）
...
## フェーズ9.7: アイコンライブラリ統一 (優先度: P3)

**ゴール**: サイト内で使用しているインラインSVGアイコンをMaterial Design Icons（@mdi/js）に統一し、保守性と一貫性を向上させる

**独立テスト**: 各コンポーネントのアイコンが正しく表示され、既存機能が維持されている
...
フェーズ2: 基盤 ←──────────────────────────────────────────┘
    ...
    │                                                 │         │
    │                                                 │         └──→ フェーズ9.6: ユーザーストーリー6A (P3) ← ユーザーメニュー
    │                                                 │                   │
    │                                                 │                   └──→ フェーズ9.7: アイコンライブラリ統一 (P3)
    │                                                 │
...
| US6A (P3)  | US6（Headerが必要）                                    | US7, US8と並列可能            |
| US7 (P3)   | US1, US2, US6（全UIコンポーネントとHeaderが必要）      | US5, US6, US6A, US8と並列可能 |
| US8 (P3)   | US6（Headerが必要）                                    | US5, US6A, US7と並列可能      |
...
- [x] T207 [P] [R003] レスポンシブサイズ定数ファイルを作成 `src/utils/responsiveClasses.ts`
- [x] T208 [R003] ThemeToggle のアイコンサイズを共通定数に置換 `src/components/ThemeToggle.tsx`
- [x] T209 [R003] HelpButton のアイコンサイズを共通定数に置換 `src/components/HelpButton.tsx`
- [x] T210 [R003] UserIcon のサイズを共通定数に置換 `src/components/UserIcon.tsx`
